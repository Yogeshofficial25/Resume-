...
const Footer: React.FC = () => {
  const socialLinks = [
    {
      ...
      url: 'https://www.linkedin.com/in/yogeshkulkarnioficial/',
      color: 'hover:text-[#00f5ff]',
    },
    {
      name: 'GitHub',
      icon: 'fab fa-github',
      url: 'https://github.com/Yogeshofficial25',
      color: 'hover:text-[#e63946]',
    },
  ];
  ...
  return (
    <footer className="py-16 px-6 border-t border-slate-100 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10 relative z-10">
        ...
        <div className="flex flex-col items-center md:items-start space-y-2">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em]">
            Protocol: Continuous Improvement
          </p>
        </div>
        ...
        <div className="flex space-x-4 order-2 md:order-3">
          {socialLinks.map((link) => (
            ...
            >
              <i
                className={`${link.icon} text-xl transition-transform group-hover:scale-110`}
              />
            </a>
          ))}
        </div>
      ...
      {/* Circuit-style footer note */}
      <div className="mt-12 text-center">
        <div className="inline-flex items-center space-x-4 opacity-20">
          <div className="h-px w-8 bg-slate-400" />
          <i className="fas fa-microchip text-xs text-slate-400" />
          <div className="h-px w-8 bg-slate-400" />
        </div>
      </div>
    </footer>
